...
export default function HardwareFormScreen({ route, navigation }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (hardwareId) {
        await hardwareAPI.update(hardwareId, payload);
      } else {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 30 }}>
      <View style={styles.header}>
        <Text style={styles.title}>{hardwareId ? 'ویرایش سخت‌افزار' : 'سخت‌افزار جدید'}</Text>
      ...
      <View style={styles.row}>
        {Object.entries(HARDWARE_TYPE_LABELS).map(([key, label]) => (
          <TouchableOpacity key={key} style={[styles.typeBtn, form.type === key && styles.typeBtnActive]} onPress={() => updateField('type', key)}>
      ...
      <View style={styles.row}>
        {Object.entries(NET_TYPE_LABELS).map(([key, label]) => (
          <TouchableOpacity key={key} style={[styles.netBtn, form.net_type === key && styles.netBtnActive]} onPress={() => updateField('net_type', key)}>
      ...
      <View style={styles.btnRow}>
        <TouchableOpacity style={[styles.saveBtn, saving && styles.disabledBtn]} onPress={handleSubmit} disabled={saving}>
          {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.saveBtnText}>{hardwareId ? 'بروزرسانی' : 'ایجاد'}</Text>}
        </TouchableOpacity>
